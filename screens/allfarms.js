...
const Allfarms = (props) =>  {
  ...
  const edit_form = (id) => {
    props.user_id({ user_id: props.user_ids.user_id, farm_id:2,update_farm_id:id })
  ...
  renderItem = ({ item, index }) => {
    ...
    return (
          ...
          <View  style={[styles.cart]} >
              ...
              <View onPress={ ()=> props.navigation.navigate('crop_health_report') }  style={[styles.box,{backgroundColor:'#359814'}]} >
                        <Text onPress={ ()=> props.navigation.navigate('crop_health_report') } style={{color:'#fff'}} >رپورٹ دیکھیں </Text>
                            <Text onPress={ ()=> props.navigation.navigate('crop_health_report') } style={{color:'#fff'}} >VIEW REPORT </Text>
                        </View>
                        <View onPress={ ()=> edit_form(item.id) } style={[styles.box,{backgroundColor:'#05422b',marginVertical:5}]} >
                        <Image
                              source={require('../assets/img/edit.png')}
                              style={{width: '15%', height: '20%',resizeMode: 'stretch',}}
                              />
                         <Text onPress={ ()=> edit_form(item.id) } style={{color:'#fff'}} >تبدیل کریں </Text>
                            <Text onPress={ ()=> edit_form(item.id) } style={{color:'#fff'}} >EDIT </Text>
                    </View>
