...
const form_2 = props => {
  ...
  if (props.flag.flag == 1) {
    switch (props.flag.s_key) {
      ...
      case 5:
        ...
        break;
      case 6:
        data.irrigation_frequency[props.flag.index].irrigation_frequesncy = props.flag.value;
        break;
      case 7:
  ...
  if (props.flag.flag == 2) {
      if(data.land_prereration_id[props.flag.index].land_prereration_id == 0){
            ...
            this.del_landpreperation(data.land_prereration_id[props.flag.index].land_prereration_id);
            data.prepared_field_irrigation.splice(props.flag.index, 1);
            data.soil_type.splice(props.flag.index, 1);
            data.laser_levelled.splice(props.flag.index, 1);
            data.levelled_date.splice(props.flag.index, 1);
  ...
  del_landpreperation = async (land_preperation_id) => {
    ...
    await fetch(URL.url + 'land_preperations/land_preperations_delete', {
      ...
      .then(res => res.json())
      .then((resjson) => {
        if (resjson.Message == 'Land Preperation of Farm Deleted Successfully') {
          alert(resjson.Message);
        }
      })
      .catch(err => {
      ...
      .then((resjson) => {
        if (resjson.Message == 'Land Preperation of Farm Get Successfully') {
          resjson.data.forEach(element => {
            ...
            data.prepared_field_irrigation.push({ prepared_field_irrigation:element.prepared_field_irrigation  });
            ...
            data.first_irrigation.push({ first_irrigation: element.first_irrigation });
            data.irrigation_frequency.push({ irrigation_frequesncy: element.irrigation_frequency });
            data.farm_distance_watercourse.push({ farm_distance_watercourse: element.farm_distance_watercourse });
          });
          setData({
            ...data,
            prepared_field_irrigation: data.prepared_field_irrigation
