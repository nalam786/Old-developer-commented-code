...
const form_5 = props => {
  ...
  const get_seedData = async () => {
    ...
    await fetch(URL.url + 'seeds/seeds_get', {
      ...
      .then(res => res.json())
      .then(resjson => {
        console.log('GET SEED DATA', resjson);
        if (resjson.Message == 'Seed of Farm Get Successfully') {
  ...
  const sendform = async () => {
    ...
    for (let i = 0; i < data.removal_of_herbs.length; i++) {
      console.log('seeeeeeeeeeeeeeed data', data.farm_id[i]);
      if (data.farm_id[i].farm_id == '') {
        let F_Data = {
          ...
          f_seed_company: data.seed_company[i].seed_company,
        ...
        console.log('Upating seeeeed', data.farm_id[i].farm_id);
        let F_Data = {
          f_farm_id: props.user_ids.update_farm_id,
          // f_removal_of_herbs: data.removal_of_herbs[i].removal_of_herbs,
          f_seed_variety: data.seed_variety[i].seed_variety,
        ...
        console.log('Seeds Update/ API Calling', F_Data);
        ...
        await fetch(URL.url + 'seeds/seeds_update', {
          ...
          .then(res => res.json())
          .then(resjson => {
            console.log('seed successfull!', resjson);
            if (resjson.Message == 'Seed Successfully Updated') {
              props.navigation.navigate('irrigation_form_edit');
            }
  ...
  const del_seedData = async id => {
    let data = {
      f_seed_id: id, //props.user_ids.update_farm_id,
    };
  ...
  if (props.flag.flag == 'seed_delete') {
    data.farm_id[props.flag.index] &&
      ...
      del_seedData(data.farm_id[props.flag.index].farm_id);
    console.log('delete indexxxxxxxxxx', props.flag.index);
