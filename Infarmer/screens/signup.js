...
const send_sms = (code) => {
   fetch('https://connect.jazzcmt.com/sendsms_url.html?Username=03040740644&Password=Flux_12345&From=Business&To='+data.phone_no+'&Message=Your Infarmer Registration code is'+code, {
    method: 'GET',
    headers: {
...
const signup = async () => {
  ...
  if(isSelected != true ){
    alert('Please check the Terms and Condetions');
    return false;
  }
  var code =''
  ...
  if(data.phone_code =='92' && data.phone !='' ){
    alert("sent_msg")
    send_sms(code)
    props.changeSignup({email:data.email,phone_code:data.phone_code,phone:data.phone_no,password:data.password,username:data.user_name,code:code})
  props.navigation.navigate('opt')
  }else if( data.email !='' ) {
    props.changeSignup({email:data.email,phone_code:data.phone_code,phone:data.phone_no,password:data.password,username:data.user_name,code:code})
    props.navigation.navigate('opt')
  }else{
      ...
      <View style={{flex: 1,flexDirection: 'column',justifyContent: 'space-between',}}>
        ...
        <View style={{alignSelf:"center" }} >
           ...
           <View style={{ flexDirection: 'row'}}>
              ...
              />
              <Text style={[styles.green_h6]}>I Agree Withe The Terms And Condetions?</Text>
           </View>
