...
// alert(props.user_ids.farm_id)
  ...
  renderForm = () => {
    ...
    // data.created_by.push({ created_by: '' });
    // data.created_at.push({ created_at: '' });
    // data.modified_by.push({ modified_by: '' });
    // data.modified_at.push({ modified_at: '' });
  ...
  const sendform = async () => {
    ...
    if(day<10 && m<10 ){
      ...
      if(m<10){
        ...
        // console.log('Land Preperation Create/Update API Calling', F_data);
    ...
    for (let i = 0; i < data.prepared_field_irrigation.length; i++) {
      ...
      await fetch(URL.url+'land_preperation/land_preperation_create', {
